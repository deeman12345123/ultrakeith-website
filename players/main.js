...
function rollCeelo() {
    ...
    if (resultEl) {
        resultEl.innerHTML = `<strong>Roll: ${dice1}-${dice2}-${dice3}</strong><br>${message}`;
    }
}
...
function resetClub() {
    ...
    const playerScoreEl = document.getElementById('playerScore');
    const dealerScoreEl = document.getElementById('dealerScore');
    const blackjackControlsEl = document.getElementById('blackjackControls');
    ...
    if (blackjackControlsEl) {
        blackjackControlsEl.innerHTML = `<button class="btn" onclick="dealCards()">Deal Cards</button>`;
    }
